...
void prims(struct Graph* graph, int visited[],struct Edge** head){
  ...
  for(int i=0; i < graph->E; i++) if(visited[graph->edge[i].src] && !visited[graph->edge[i].dest]) {push(head, graph->edge[i]);visited[graph->edge[i].dest] = 1;}
}
int main() {
  int V= 5, E=7,visited[V];
  memset(visited,0, V * sizeof(int));
  struct Graph* graph = createGraph(V, E);
  int edges[7][3] = {{0, 1, 2},{0, 3, 6},{1, 2, 3},{1, 3, 8},{1, 4, 5},{2, 4, 7},{3, 4, 9}};
